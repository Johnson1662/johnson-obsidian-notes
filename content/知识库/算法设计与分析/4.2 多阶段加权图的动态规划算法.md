...
![](../数理基础课程/大物2A/assets/作业题/image%202.png)
...
4. **逆序计算最短距离**：- 初始化：( c(F) = 0 )。- **阶段3**：- ( c(D) = c(F) + 2 = 2 )。- ( c(E) = c(F) + 4 = 4 )。- **阶段2**：- B的邻接节点为D和E：
   ( $c(B) = \min { c(D) + 1, c(E) + 4 } = \min { 3, 8 } = 3$ )（选择D）。- C的邻接节点为D和E：
   ( $c(C) = \min { c(D) + 5, c(E) + 3 } = \min { 7, 7 } = 7$)（选择D或E）。
5. **回溯路径**：- **阶段1**：- A的邻接节点为B和C：
   ( $c(A) = \min { c(B) + 3, c(C) + 2 } = \min { 6, 9 } = 6$)（选择B）。- A → B → D → F。
