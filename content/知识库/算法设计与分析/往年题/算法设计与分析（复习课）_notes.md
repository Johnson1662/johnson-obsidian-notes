...
```
Algorithm: Interval-Partitioning(A)
1.  将活动集合 A 按照开始时间 s_i 从小到大进行排序，记新序列为 I_1, ..., I_n
2.  初始化一个空的最小堆 H  // 用于动态维护并快速获取每个已开辟教室的“最早空闲时间”
3.  初始化教室计数器 d = 0
4.  
5.  for j = 1 to n do
6.      if 最小堆 H 不为空 and 活动 I_j 的开始时间 s_j >= H 的堆顶元素值 then
7.          room_id = 弹出最小堆 H 的堆顶元素  // 复用当前最早释放的教室
8.          将活动 I_j 分配给该教室 room_id
9.          将更新后的教室释放时间 f_j 重新插入最小堆 H
10.     else
11.         d = d + 1  // 当前所有教室都正忙，必须开辟新教室
12.         将活动 I_j 分配给新教室 Room_d
13.         将新教室的释放时间 f_j 插入最小堆 H
14.     end if
15. end for
16. 
17. return 所有活动的教室分配方案与总教室数 d
```
