...
### 1. 问题定义与贪心规则
* **已知条件**：汽车满油最大行驶里程为 $n$。所有加油站按先后顺序编号为 $1, 2, \dots, m$（其中 $1$ 极为起点，$m$ 为终点）。
* **贪心规则**：每次加油时，选择当前油量限制内**能够安全到达的最远**的一个加油站。
...
#### ① 假设存在最优解，且按照规则排序
假设存在一个全局最优解（加油次数最少）记为 $O$。将 $O$ 选择的加油站序列升序排列：
$$O = \{o_1, o_2, \dots, o_k\}$$
同样，将贪心算法得到的解序列升序排列：
$$G = \{g_1, g_2, \dots, g_k, \dots\}$$
设 $O$ 和 $G$ 从左到右第一个不相同的元素（最左元素）发生在第 $i$ 次加油，即：
$$o_1 = g_1, \dots, o_{i-1} = g_{i-1}, \quad \text{但} \quad o_i \neq g_i$$

#### ② 利用贪心规则替换最左元素
根据贪心规则的定义，$g_i$ 是从前一步共同站 $g_{i-1}$（即 $o_{i-1}$）出发能走的最远距离。既然最优解 $O$ 在此处做出了不同选择，则必然有：
$$o_i < g_i$$
我们利用贪心规则，将最优解 $O$ 中的最左不同元素 $o_i$ 替换为 $g_i$，构造出一个新的序列 $O'$：
$$O' = \{o_1, o_2, \dots, o_{i-1}, \mathbf{g_i}, o_{i+1}, \dots, o_k\}$$

#### ③ 反证法证明替换后仍为最优
我们要证明新序列 $O'$ 在替换后依然合法（即汽车在后续路段不会断油）。

* **反设**：假设替换后的方案 $O'$ 是不合法的，即汽车从更远的 $g_i$ 站加满油出发，无法安全到达原定的下一站 $o_{i+1}$。用数学语言表示为：
    $$\text{从 } g_i \text{ 到 } o_{i+1} \text{ 的距离} > n$$
* **导向矛盾**：因为在步骤 ② 中已知 $o_i < g_i$（即 $g_i$ 比 $o_i$ 更靠近 $o_{i+1}$），所以显而易见：
    $\text{从 } o_i \text{ 到 } o_{i+1} \text{ 的距离} > \text{从 } g_i \text{ 到 } o_{i+1} \text{ 的距离}$
    结合反设的不等式，可递推导出：
    $\text{从 } o_i \text{ 到 } o_{i+1} \text{ 的距离} > n$
    这意味着，原方案 $O$ 中汽车从 $o_i$ 出发也根本开不到 $o_{i+1}$，方案 $O$ 本身就会中途断油。
* **得出结论**：这与“已知 $O$ 是一个合法的最优解”相**矛盾**。故反设不成立，$O'$ 方案完全合法。由于 $O'$ 的加油次数仍为 $k$，因此替换后的方案 $O'$ 仍为最优解。

#### ④ 形成子问题及其最优质
通过上述替换，我们成功将最优解在第 $i$ 位纠正为了贪心选择。此时，问题规约（缩小）为了一个规模更小的**子问题**：

> “从新站点 $g_i$ 出发，如何用最少的加油次数到达终点？”

我们可以重新返回步骤 ② 和 ③，继续寻找并替换下一个最左不同元素（$o_{i+1}$ 与 $g_{i+1}$）。通过不断的子问题规约与迭代替换，最优解 $O$ 可以在加油次数不增加的前提下被完全改造成贪心解 $G$。
