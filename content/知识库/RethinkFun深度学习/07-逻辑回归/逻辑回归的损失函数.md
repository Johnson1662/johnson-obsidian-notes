---
title: 逻辑回归的损失函数
source: https://www.rethink.fun/chapter7/%E9%80%BB%E8%BE%91%E5%9B%9E%E5%BD%92%E7%9A%84%E6%8D%9F%E5%A4%B1%E5%87%BD%E6%95%B0.html
chapter: 07-逻辑回归
...
$LogisticRegssion(x) = Sigmoid(wx+b)=\frac{1}{1+e^{-(wx+b)}}$LogisticRegssion(x)=Sigmoid(wx+b)=1+e−(wx+b)1​
...
我们用$y$y表示Label值，用$\hat{y}$y^​表示预测值。

则二分类交叉熵损失函数BCELoss(Binary Cross Entropy Loss)分为两种情况。

分别是当$y=1$y=1时：
$BCELoss=-log(\hat{y})$BCELoss=−log(y^​)

当$y=0$y=0时：
$BCELoss=-log(1-\hat{y})$BCELoss=−log(1−y^​)
...
$BCELoss(y,\hat{y}) = -[ylog\hat{y}+(1-y)log(1-\hat{y})]$BCELoss(y,y^​)=−[ylogy^​+(1−y)log(1−y^​)]
