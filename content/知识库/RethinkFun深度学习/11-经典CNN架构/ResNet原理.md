---
title: ResNet原理
source: https://www.rethink.fun/chapter11/ResNet%E5%8E%9F%E7%90%86.html
chapter: 11-经典CNN架构
...
之前，我们增加一层网络，是希望这一层能提取到更好的特征，提升网络性能。这一层的输入是x，我们希望学到一个函数$H(x)$H(x)，这个函数的输出就是新的特征。

![1047.png](http://rethink.fun/imgs/1047.png)

ResNet作者的思想是，网络增加一层后的输出由两部分构成，一部分是输入x，一部分是新增加层对x做的修改。对x做的修改部分叫做残差部分，用$F(x)$F(x)表示。增加的这一层最终输出的特征为$F(x)+x$F(x)+x。期望$F(x)+x=H(x)$F(x)+x=H(x)。这种通过人为修改网络架构的方式，将对x的映射分为两部分：x本身和对x改变的部分，让模型更容易学到恒等映射。
