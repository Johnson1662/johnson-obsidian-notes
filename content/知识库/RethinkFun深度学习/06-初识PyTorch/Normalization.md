---
title: Normalization
source: https://www.rethink.fun/chapter6/Normalization.html
...
假设我们构造了如下的数据，这个数据是关于对外卖送餐时间的数据。对于送餐时间（分钟）。影响因素有两个，一个是路上红绿灯的数量，一个是饭店距你的距离（米）。构造数据时，我们假设：
 $time=2*lights+0.01*distance+5$time=2∗lights+0.01∗distance+5
...
可以发现对于lights权重$w_0$w0​的梯度值为8600，对于distance权重$w_1$w1​的梯度值为5876040。$w_1$w1​的梯度大概是$w_0$w0​梯度的1000倍。
这是因为$w_1$w1​这个权重是作用在distance上的，$w_0$w0​这个权重是作用在lights上的。distance的值大概是lights值的1000倍。
作用到最终loss函数上，进行同样的改变，$w_1$w1​的变化对loss值的影响就是$w_0$w0​的1000倍。所以造成了两个权重的梯度值相差了1000倍。

初始化时，$w_0$w0​和$w_1$w1​都是1，最终我们希望$w_0$w0​能调整为2，$w_1$w1​能调整到0.01。我们看到$w_1$w1​的梯度值非常大，如果学习率稍微大一些，1减去学习率乘以梯度值，就跨过了需要调整到的0.01。这就是为什么学习率需要设置的很小。

为了迁就对$w_1$w1​的调整，学习率必须设置的很小，但是这会导致对$w_0$w0​每次的调整太少，导致训练非常慢。这就是为什么当loss降到7左右就很难再下降了。

根本原因就是对$w_0$w0​和$w_1$w1​的训练，它们共用学习率，但是因为它们对应feature的取值范围不同，导致他们对loss函数的影响不同，进而导致它们对loss的梯度的取值范围不同。
