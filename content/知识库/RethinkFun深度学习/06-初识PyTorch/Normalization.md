...
可以发现对于lights权重w0w_0w0​的梯度值为8600，对于distance权重w1w_1w1​的梯度值为5876040。w1w_1w1​的梯度大概是w0w_0w0​梯度的1000倍。
这是因为w1w_1w1​这个权重是作用在distance上的，w0w_0w0​这个权重是作用在lights上的。distance的值大概是lights值的1000倍。
作用到最终loss函数上，进行同样的改变，w1w_1w1​的变化对loss值的影响就是w0w_0w0​的1000倍。所以造成了两个权重的梯度值相差了1000倍。

初始化时，w0w_0w0​和w1w_1w1​都是1，最终我们希望w0w_0w0​能调整为2，w1w_1w1​能调整到0.01。我们看到w1w_1w1​的梯度值非常大，如果学习率稍微大一些，1减去学习率乘以梯度值，就跨过了需要调整到的0.01。这就是为什么学习率需要设置的很小。

为了迁就对w1w_1w1​的调整，学习率必须设置的很小，但是这会导致对w0w_0w0​每次的调整太少，导致训练非常慢。这就是为什么当loss降到7左右就很难再下降了。

根本原因就是对w0w_0w0​和w1w_1w1​的训练，它们共用学习率，但是因为它们对应feature的取值范围不同，导致他们对loss函数的影响不同，进而导致它们对loss的梯度的取值范围不同。
