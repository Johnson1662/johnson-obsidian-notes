---
title: 权重衰减
source: https://www.rethink.fun/chapter9/权重衰减.html
chapter: 09-优化与正则化
...
wt=wt−lr×gw−lr×λwtw_t=w_t-lr\times g_w-lr\times \lambda w_twt​=wt​−lr×gw​−lr×λwt​
...
L=Lerror+λ2w2L=L_{error}+\frac{\lambda}{2}w^2L=Lerror​+2λ​w2

更新参数w,等于w减去学习率lr乘以L对w的偏导数：

w=w−lr∂L∂ww=w-lr\frac{\partial L}{\partial w}w=w−lr∂w∂L​

=w−lr∂Lerror∂w−lr×λw=w-lr\frac{\partial L_{error}}{\partial w}-lr\times \lambda w=w−lr∂w∂Lerror​​−lr×λw

**权重衰减**

L=LerrorL=L_{error}L=Lerror​

更新参数w：

w=w−lr∂Lerror∂w−lr×λww=w-lr\frac{\partial L_{error}}{\partial w}-lr\times \lambda ww=w−lr∂w∂Lerror​​−lr×λw
