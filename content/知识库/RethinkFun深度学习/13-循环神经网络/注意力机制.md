---
title: 注意力机制
source: https://www.rethink.fun/chapter13/%E6%B3%A8%E6%84%8F%E5%8A%9B%E6%9C%BA%E5%88%B6.html
chapter: 13-循环神经网络
...
其中$\alpha_i$αi​是标量，表示在翻译当前token时对$h_i$hi​的注意力，且$\sum_{i=1}^{t} \alpha _i=1$∑i=1t​αi​=1。$h_i$hi​为第i个时间步，也就是第i个英文token输出的隐状态向量。

有了注意力向量，将它与每个时间步输入的中文token的embedding进行拼接作为输入就实现了带注意力的Encoder-Decoder RNN了。

我们就剩下一个问题了，那就是如何计算注意力权重$\alpha_i$αi​?
...
假设Encoder输入3个英文token，分别为$x_0,x_1,x_2$x0​,x1​,x2​。初始隐状态为$h_0$h0​，三个时间步对应的隐状态分别为$h_1,h_2,h_3$h1​,h2​,h3​。将$h_3$h3​作为解码器的隐状态输入，为了区分，Decoder的隐状态用s表示。所以$h_3=s_0$h3​=s0​。Decoder每一步生成的中文token，作为下一时间步的输入。并且中间产生隐状态$s_1,s_2,s_3$s1​,s2​,s3​。这个图里边是没有增加注意力机制的Encoder-Decoder RNN，这里我们只是定义了符号，接下来我们看如何计算注意力权值。

我们以Decoder第一个时间步的注意力向量$att_0$att0​为例，需要生成对$h_1,h_2,h_3$h1​,h2​,h3​的注意力权重值$\alpha_1,\alpha_2,\alpha_3$α1​,α2​,α3​，并且需要保证$\alpha_1+\alpha_2+\alpha_3=1$α1​+α2​+α3​=1。则注意力向量$att_0$att0​就等于：
...
我们定义一个简单的全连接神经网络，假设它只有一层，输出层，输出层只有一个神经元。首先将$s_0$s0​分别和$h_1,h_2,h_3$h1​,h2​,h3​进行拼接，得到3个输入向量，然后分别经过全连接神经网络，得到3个标量的logits值，再对这3个logits值应用softmax，就得到了$\alpha_1,\alpha_2,\alpha_3$α1​,α2​,α3​的具体注意力权重值了。

Decoder第二个时间步的注意力向量为$att_1$att1​，它也需要计算当前时间步对$h_1,h_2,h_3$h1​,h2​,h3​的注意力权重值$\alpha_1,\alpha_2,\alpha_3$α1​,α2​,α3​。做法和上一个时间步一样，将$s_1$s1​分别和$h_1,h_2,h_3$h1​,h2​,h3​进行拼接，得到3个输入向量，然后这3个输入向量分别经过全连接神经网络，得到3个logits值，再对这3个logits值应用softmax，就得到第二个时间步对$h_1,h_2,h_3$h1​,h2​,h3​的注意力权重：$\alpha_1,\alpha_2,\alpha_3$α1​,α2​,α3​了。
