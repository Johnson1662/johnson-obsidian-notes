...
![interrupt](/08-ECF/interrupt.png)
...
![common_exception](/08-ECF/common_exception.png)
...
（注意和过程调用有出入，第 4 个参数是 `%r10`，而不是过程调用中的 `%rcx`，[回顾](https://slide.huh.moe/02/13)）
...
![concurrent](/08-ECF/concurrent.png)
...
![private_address_space](/08-ECF/private_address_space.png)
...
![context_switch](/08-ECF/context_switch.png)
...
当 Unix **系统级函数遇到错误** 时，它们通常会 **返回 `-1`，并设置全局整数变量 `errno`** 来表示什么出错了。
...
![fork_example_1](/08-ECF/fork_example_1.png)
...
![fork_example_2_code](/08-ECF/fork_example_2_code.png)


![fork_example_2_ans](/08-ECF/fork_example_2_ans.png)
...
`options` 是基于位向量实现的，所以可以使用 `|` 来组合多个选项。 `0001 | 0010 = 0011`
...
![wait_example](/08-ECF/wait_example.png)
...
![argv_and_envp](/08-ECF/argv_and_envp.png)


![user_stack](/08-ECF/user_stack.png)
...
![sig_handler](/08-ECF/sig_handler.png)
...
![process_group](/08-ECF/process_group.png)
...
![sig_handler_double](/08-ECF/sig_handler_double.png)
...
- 不可重入，有缓冲区
- 考虑信号处理函数序列 
 $\text{handler}_1 \to \text{handler}_2 \to \text{handler}_1$
