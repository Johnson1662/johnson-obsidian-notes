...
**通俗解释**：就像员工入职（创建）→ 等待分配工作（就绪）→ 正在工作（运行）→ 可能因为等材料暂停（阻塞）→ 完成工作离职（终止）。

### 1.5 创建进程与线程的开销对比

| 平台 | fork() 耗时 | pthread_create() 耗时 |
|------|-------------|----------------------|
| AMD 2.4GHz Opteron (8核) | 41.07s | 0.66s |
| IBM 1.9GHz POWER5 (8核) | 64.24s | 1.75s |
| Intel 2.4GHz Xeon (2核) | 54.95s | 1.64s |

**结论**：创建线程比创建进程快约**30-60倍**，因为线程共享进程资源，无需分配独立的内存空间和系统资源。

### 1.6 线程池

**线程池（Thread Pool）**：
- 维护多个线程，等待调度器分配可并发执行的任务
- **通俗理解**：就像出租车调度中心，保持一批待命的出租车（线程），有乘客（任务）来了就派车，不用每次都买车（创建线程）
- **优点**：
  - 避免频繁创建/销毁线程的开销
  - 保证内核充分利用
  - 防止过分调度
- **适用场景**：Web服务器处理请求、数据库连接池、任务队列处理
...
void* Thread_sum(void* rank) {
    ...
    double factor;
    long long i;
    long long my_n = n/thread_count;  // 每个线程处理的数据量
    long long my_first_i = my_n * my_rank;  // 起始位置
    long long my_last_i = my_first_i + my_n;  // 结束位置
    double my_sum = 0.0;  // 线程私有变量
...
**性能对比**（使用busy-waiting vs mutex）：

| 线程数 | Busy-Waiting耗时 | Mutex耗时 |
|--------|------------------|-----------|
| 1 | 2.90s | 2.90s |
| 4 | 0.73s | 0.73s |
| 16 | 0.50s | 0.38s |
...
**结论**：线程数较多时，**互斥锁性能明显优于忙等待**，因为忙等待浪费CPU时间。
...
- 线程生命周期 → **PPT 03 第97-99页**
- 开销对比表格 → **PPT 03 第86-88页**

### 💡 学习建议
1. **理解概念**：先理解线程、进程、同步等基本概念
2. **动手实践**：编写简单的Pthread程序，如积分法求π
3. **性能分析**：对比不同同步方法的性能差异
4. **调试技巧**：学会使用gdb调试多线程程序
