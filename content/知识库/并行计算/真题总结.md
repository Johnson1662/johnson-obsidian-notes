...
#### UMA（均匀存储访问）
每个处理器等同地访问统一存储空间
![](assets/真题总结/file-20260428214350253.png)
#### NUMA（非均匀存储访问）
- 存储全局统一编址，每个处理器均可直接访问
- 处理器访问本地存储比远程存储快
![](assets/真题总结/file-20260428214355639.png)
## 3 集群
...
## 5 互斥锁
一种用于多线程编程中防止多个线程同时访问共享资源（临界区）的机制 。它确保在任何时刻，**只有一个线程**能够持有锁并进入**临界区（访问共享数据的代码段，任意时刻至多一个线程可执行）** 执行代码，从而避免**竞态条件（当多个线程同时访问共享内存，且最终结果取决于线程执行顺序时，称为竞态条件）** 
## 6 加速比
...
## 8 MPI编程模型（Message Passing Interface）
