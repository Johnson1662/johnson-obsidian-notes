...
```pseudocode 
MPI_Init() my_rank ← MPI_Comm_rank(MPI_COMM_WORLD) p ← MPI_Comm_size(MPI_COMM_WORLD)

if my_rank == 0 then n ← 输入 n end if
...
// 公式验证 formula ← [n(n+1)/2]² print "公式验证: " + formula end if
