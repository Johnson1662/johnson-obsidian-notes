...
$$
\begin{array}{l}
\text{\textbf{MPI 并行求 } 1^3 + 2^3 + \dots + n^3} \\[1em]
\text{MPI\_Init()} \\
\text{my\_rank} \leftarrow \text{MPI\_Comm\_rank(MPI\_COMM\_WORLD)} \\
\text{p} \leftarrow \text{MPI\_Comm\_size(MPI\_COMM\_WORLD)} \\[1em]
\If{\text{my\_rank} = 0} \\
\quad n \leftarrow \text{输入或命令行参数} \\
\text{MPI\_Bcast}(n, \text{root}=0) \\[1em]
\text{// 任务划分} \\
\text{chunk} \gets n / p, \quad \text{remainder} \gets n \bmod p \\
\begin{cases}
\text{start} \gets \text{my\_rank} \times (\text{chunk}+1) + 1 \\
\text{end} \gets \text{start} + \text{chunk} & \text{if my\_rank} < \text{remainder} \\
\text{start} \gets \text{my\_rank} \times \text{chunk} + \text{remainder} + 1 \\
\text{end} \gets \text{start} + \text{chunk} - 1 & \text{otherwise}
\end{cases} \\[1em]
\text{local\_sum} \gets 0 \\
\text{for } i = \text{start} \text{ to } \text{end} \text{ do} \\
\quad \text{local\_sum} \gets \text{local\_sum} + i^3 \\
\text{MPI\_Reduce}(\text{local\_sum}, \text{total\_sum}, \text{MPI\_SUM}, \text{root}=0) \\[1em]
\If{\text{my\_rank} = 0} \\
\quad \text{输出 } 1^3 + 2^3 + \dots + n^3 = \text{total\_sum} \\
\quad \text{formula} \gets \left( \dfrac{n(n+1)}{2} \right)^2 \\
\quad \text{输出公式验证: } \text{formula}
\end{array}
$$
```pseudocode 
MPI_Init() 
