...
### 1. 内存延迟（Latency）与峰值速度计算

这类题目考查你是否理解“内存墙”对计算速度的限制。

- **典型题型：** 给出处理器的时钟频率、计算部件能力以及DRAM的延迟时间，求该平台的峰值计算速度。
- **核心逻辑：**
    - **处理器峰值：** 处理器主频 $\times$ 每个时钟周期执行的指令数。
    - **访存受限速度：** 如果没有Cache，每次计算都要等待内存延迟。速度 = 1 / (内存延迟)。
- **真题示例：**
    - 条件：处理器主频 1GHz（周期1ns），内存延迟 100ns。计算向量点积时，每次浮点运算需要取一次数据。
    - 计算：由于没有Cache，处理器每100ns才能完成一次计算，其实际速度仅为 10 MFLOPS，远低于其 4 GFLOPS 的理论峰值。

### 2. 缓存（Cache）对性能的提升计算

这类题目通常结合矩阵运算，考查你对**时间本地性**和**空间本地性**的理解。

- **典型题型：** 比较“有Cache”和“无Cache”两种情况下，完成特定规模运算（如 $32 \times 32$ 矩阵乘法）的总时间。
- **核心公式：**
    - **总时间 = 数据加载时间 + 实际计算时间**。
    - **加载时间：** 数据总量 / 内存带宽（或访存次数 $\times$ 延迟）。
    - **计算时间：** 总运算步数 / (主频 $\times$ 并行度)。
- **结论判定：** 引入Cache后，加载时间显著缩短，整体 MFLOPS 会大幅提升。

### 3. 内存带宽（Bandwidth）与块大小（Block Size）影响

这类题目考查如何通过增加“访存粒度”来掩盖延迟。

- **典型题型：** 将内存交换的块大小（Block Size）从 1 个字增加到 4 个字，求峰值速度的变化。
- **计算方法：**
    - 如果块大小增加到 4，意味着一次延迟时间内可以取出 4 个连续数据。
    - 如果算法满足**空间本地性**（即连续使用这 4 个数据），则等效于访存速度提升了 4 倍。
    - **示例：** 原本每 100ns 处理 1 个数，现在每 100ns 处理 4 个数，速度从 10 MFLOPS 提升到 40 MFLOPS。
