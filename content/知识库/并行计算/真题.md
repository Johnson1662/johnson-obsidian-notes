...
## 参考例题
**题目背景：** 已知某处理器的主频为 **1GHz**（每个时钟周期为 1ns），内部设有 2 个乘加（Multiply-Add）部件，每个时钟周期理论上最多能执行 **4 条** 浮点指令（理论峰值 4 GFLOPS）。与之相连的 DRAM 内存访问延迟为 **100ns**。

**请根据以上条件回答下列问题：**

**（1）计算访存受限下的速度：** 若该处理器**没有高速缓存（Cache）**，且内存交换的**块大小（Block Size）为 1 个字**。现执行两个长向量的点积运算（$a \cdot b = \sum a_i b_i$），已知每次浮点运算都需要取一次数据。

- ① 请问在该平台上执行此点积运算的**实际速度**是多少？（以 MFLOPS 为单位）
- ② 此时处理器的效率（实际速度 / 理论峰值）是多少？

**（2）计算高速缓存对性能的提升：** 在上述硬件基础上，增加一个容量为 **32KB** 的高速缓存（Cache），其延迟可忽略不计（1ns）。

- 计算任务：完成 $32 \times 32$ 的**矩阵乘法**（$C = A \times B$）。
- 已知条件：$32 \times 32$ 的矩阵 A 和 B 总计占用 **2K 个字**，可完整放入 Cache。矩阵乘法的运算步数遵循 $2n^3$ 公式。
- ① 请计算完成该矩阵乘法所需的**总时间**（包含从内存加载数据到 Cache 的时间 + 在 Cache 内计算的时间）。
- ② 请计算该算法在该系统下的**有效计算速度**（MFLOPS）。

**（3）计算内存带宽的影响：** 回到第（1）小题的**向量点积**运算（无 Cache 情况）。

- 若将内存交换的**块大小（Block Size）从 1 个字增加到 4 个字**，且假设向量数据在内存中是线性排列的（符合空间本地性）。
...
为了让你彻底吃透这道题，我们现场拆解一下得分点：

#### 第一问：考查“访存延迟” (Latency)

- **逻辑**：CPU 强没用，内存慢是死穴。
- **计算**：每 100ns 才能拿回一个数做一个 FLOP，所以 $1 \text{ FLOP} / 100\text{ns} = \mathbf{10 \text{ MFLOPS}}$。
- **结论**：实际速度仅为理论峰值的 **0.25%**（4000 MFLOPS 的四百分之一）。

#### 第二问：考查“时间本地性” (Cache/Temporal Locality)

- **核心步数**：$2 \times 32^3 = \mathbf{64,000 \text{ 步}}$。
- **加载时间**：取 2K 个字 $\times$ 100ns = $200,000\text{ns} = \mathbf{200 \mu s}$。
- **计算时间**：64,000 步 / (每纳秒 4 步) = $16,000\text{ns} = \mathbf{16 \mu s}$。
- **最终结果**：总时间 $216\mu s$。速度约为 **303 MFLOPS**。相比无 Cache 情况，速度提升了 **30 倍**！

#### 第三问：考查“空间本地性” (Bandwidth/Spatial Locality)
