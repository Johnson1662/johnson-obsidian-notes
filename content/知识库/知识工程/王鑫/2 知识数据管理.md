...
2. 以下关于SPARQL 和Cypher 语言的比较，哪一项是正确的？
A. SPARQL 专门用于属性图操作，Cypher 用于RDF 图。
B. SPARQL 与Cypher 都无法进行数据更新操作。
C. SPARQL 使用三元组模式匹配数据，Cypher 使用图形模式
（ASCII 艺术表示法）匹配数据。
73
D. SPARQL 和Cypher 必须使用相同的前缀语法才可执行查
询。
