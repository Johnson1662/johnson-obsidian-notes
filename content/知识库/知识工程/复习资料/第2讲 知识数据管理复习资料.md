...
RDF 图就是很多三元组组成的图。每个三元组是一条“主语-谓词-宾语”的边。RDF 适合语义万维网和知识图谱，但如果要给边加属性，普通 RDF 需要额外建一个“表示这条边的节点”，RDF* 则更直接。
...
1. $V$：顶点有限集合。
2. $E$：边有限集合。
3. $\rho$：将边关联到顶点对，例如 $\rho(e)=(s,t)$，其中 $s$ 是源点、$t$ 是目标点。
4. $\lambda$：为顶点或边赋予标签。
5. $\sigma$：为顶点或边关联属性。
