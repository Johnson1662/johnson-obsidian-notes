...
\sigma(v_2,标签)&=\text{"Titanic (1997 film)"} \\
...
| 类别 | 函数/操作符 | 例子 |
| --- | --- | --- |
| 逻辑化 & Comparisons | `!`, `&&`, `||`, `=`, `!=`, `<`, `<=`, `>`, `>=`, `IN`, `NOT IN` | `?hasPermit || ?age < 25` |
...
Property paths allow 三元组模式s to match arbitrary-length paths through a graph。
...
第一条三元组找到电影资源 `?x1`；第二条三元组再查这个电影资源的 标签 `?x2`。共同变量 `?x1` 把两条模式连接起来。

#### 常见考法：

- 问为什么这是“基本图模式”：因为多条 三元组模式 通过变量 join。
- 给查询，让你画匹配链。
...
| x | x.标签 |
| --- | --- |
| `{"length":195,"标签":"Titanic (1997 film)", "budget":200000000.0}` | `"Titanic (1997 film)"` |

#### 通俗解释

`(:Director {name: "James Cameron"})` 匹配名字为 James Cameron 的导演节点；`-[:directs]->` 匹配其指向电影的执导边；`x:Movie` 把电影节点绑定为变量 `x`；最后返回整个电影节点和其 标签 属性。
...
`name` 是导演节点属性；`标签` 是电影节点属性。不要把 `rdfs:标签` 和属性图中的 `x.标签` 混为同一种语法。
