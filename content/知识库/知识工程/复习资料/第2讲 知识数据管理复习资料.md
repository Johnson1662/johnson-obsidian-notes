...
![[assets/345f722b6965d6669a33128770533ddd62a753e209adab0843ac2d8e3887abe1.jpg]]
...
边属性例子：詹姆斯·卡梅隆在《泰坦尼克号》中 `acts_in`，角色值是 `"Steerage Dancer"`（三等舱舞者）。
...
- 元数据语句成为一等公民。
- 允许一个 三元组 出现在另一个 三元组 的 主语 或 宾语 位置。
- 嵌入三元组 本身也可继续作为 元数据三元组，支持递归。

### 通俗解释

RDF 图就是很多三元组组成的图。每个三元组是一条“主语-谓词-宾语”的边。RDF 适合语义万维网 和知识图谱，但如果要给边加属性，普通 RDF 需要额外建一个“表示这条边的节点”，RDF* 则更直接。

### 常见考法：

- 给自然语言或图，写 RDF 三元组集合。
- 给 RDF 三元组集合，画 RDF 图。
- 问 RDF 为什么不方便表示边属性，以及 具体化 / RDF* 如何解决。

### 易错点 / 答题模板

> RDF 图是 RDF 三元组的有限集合。三元组形如 `(s,p,o)`，表示资源 s 通过属性/关系 p 指向资源或字面值 o。RDF 本身没有内置边属性，若要表示边上的属性，可使用 具体化 将三元组本身作为资源描述，或使用 RDF* 的嵌入三元组表示。
...
1. $V$：顶点有限集合。
2. $E$：边有限集合。
3. $\rho$：将边关联到顶点对，例如 $\rho(e)=(源点,目标点)$。
4. $\lambda$：为顶点或边赋予标签。
5. $\sigma$：为顶点或边关联属性。
...
![[assets/932ef1e85521ebca27fdabe773648274cbfdd7e9ce427e0ebdd8d39268547149.jpg]]

另一个社交图例：

![[assets/1d2d66a4846f259033b68d1bab854c20d2b62264284dbdc51e286dcc914a386a.jpg]]

### 通俗解释

属性图比 RDF 更直接：节点和边都可以有标签和属性。例如 `acts_in` 这条边可以直接有 `role = "Jack Dawson"`，不用像 RDF 那样 具体化。
...
- 变量必须以 `?` 开头。
- 字面值 和 IRI 不同，`"Titanic"` 是字面值，`dbr:Titanic` 是资源。
...
含义：左连接。保留 A 的所有解；如果 B 可匹配，则补充 B 的变量绑定。
...
| 类别 | 函数/操作符 | 例子 |
| --- | --- | --- |
| 逻辑运算与比较运算 | `!`, `&&`, `||`, `=`, `!=`, `<`, `<=`, `>`, `>=`, `IN`, `NOT IN` | `?hasPermit || ?age < 25` |
| 条件函数 | `EXISTS`, `NOT EXISTS`, `IF`, `COALESCE` | `NOT EXISTS { ?p foaf:mbox ?email }` |
| 数学 | `+`, `-`, `*`, `/`, `abs`, `round`, `ceil`, `floor`, `RAND` | `?decimal * 10 > ?minPercent` |
| 字符串 | `STRLEN`, `SUBSTR`, `UCASE`, `LCASE`, `CONTAINS`, `REGEX` 等 | `STRLEN(?description) < 255` |
| 日期/时间 | `now`, `year`, `month`, `day`, `hours`, `minutes`, `seconds` | `month(now()) < 4` |
| 测试函数 | `isURI`, `isBlank`, `isLiteral`, `isNumeric`, `bound` | `isURI(?person) || !bound(?person)` |
| 访问函数 | `str`, `lang`, `datatype` | `lang(?title) = "en"` |
...
属性路径允许三元组模式匹配图中的任意长度路径。

操作符：

| 构造 | 含义 |
| --- | --- |
| `path1/path2` | 路径连接，先走 `path1` 再走 `path2` |
| `^path1` | 反向路径，从 宾语 到 主语 |
| `path1|path2` | 路径并集，二选一 |
| `path1*` | 零次或多次 |
| `path1+` | 一次或多次 |
| `path1?` | 可选，一次或零次 |
| `!uri` | 任意谓词，除了 uri |
...
第一条三元组找到电影资源 `?x1`；第二条三元组再查这个电影资源的 `label`（片名/标签）并绑定为 `?x2`。共同变量 `?x1` 把两条模式连接起来。

#### 常见考法：

- 问为什么这是“基本图模式”：因为多条三元组模式通过变量连接。
- 给查询，让你画匹配链。
...
`?x1` 是被约束的中心节点；如果三条模式的变量名写不一致，就不能正确连接。
...
### 4.2 Cypher 示例 6：查询詹姆斯·卡梅隆执导的电影及片名
...
`(:Director {name: "James Cameron"})` 匹配名字为“詹姆斯·卡梅隆”的导演节点；`-[:directs]->` 匹配其指向电影的执导边；`x:Movie` 把电影节点绑定为变量 `x`；最后返回整个电影节点和其 `label` 属性。
...
- 与 SPARQL 查询 4 互译。
- 改过滤条件。
- 问为什么 `acts_in` 的箭头是从演员指向电影，因此查询写成 `<-[:acts_in]-`。
...
查询 8：查询 “San Zhang”（张三）直接和间接认识的人。
...
Markdown 和 PDF 中未看到独立标题为“作业”的页面。结合提纲，最可能需要复习的是课件中 SPARQL 与 Cypher 的查询练习：
