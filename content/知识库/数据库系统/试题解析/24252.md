...
1）给事务 $T1$ 和 $T2$ 增加锁（共享锁 sli (X) 或排他锁 xli (X)）和解锁指令 ui (X)，使它们遵循两阶段封锁协议（3 分）。 $T1： sl1​(A);R1​(A);xl1​(B);R1​(B);W1​(B);u1​(A);u1​(B)$
$T2： sl2​(B);R2​(B);sl2​(A);R2​(A);xl2​(A);W2​(A);xl2​(B);W2​(B);u2​(B);u2​(A)$
