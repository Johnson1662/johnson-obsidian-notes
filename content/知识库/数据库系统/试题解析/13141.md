...
FROM SC
GROUP BY cno
HAVING COUNT(*) >= 2  --- HAVING 语句有时在SELECT语句之前执行
ORDER BY cno
```
...
FROM Student AS s,Course AS c,SC AS sc
...
Ep*->Dp*
![](assets/13141/file-20260513153538167.png)
...
SELECT e.eno,e.name,r.pno,p.pname 
FROM employee e, project p,responsible r
WHERE e.eno = r.eno AND r.pno = p.pno

--- 创建索引
CREATE INDEX eno_idx ON responsible(eno)
```
不可更新，该视图定义中涉及多个表的实例
目的是加快视图查询速度
