...
![](assets/13141/file-20260512142127125.png)好的，这是将图片内容转换为Markdown格式：
...
编写 SQL 语句完成下列查询:
![](assets/13141/file-20260512142243763.png)下面是将图片内容转换为Markdown格式：
...
```sql
INSERT INTO Course (cno, cname, ccredits, tno)
VALUES (5, '高级数据库', 2, '05004');
```
...
```sql
DELETE FROM Course
WHERE tno = (SELECT tno FROM Teacher WHERE tname = '赵小兰');
```
*(假设 `Teacher` 表包含 `tno` 和 `tname` 字段，且 `Course` 表通过 `tno` 关联教师)*
