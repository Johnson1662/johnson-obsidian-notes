...
在 Linux 中，文件实际上可以看做是**字节的序列**。所有的 I/O 设备以及内核都是用文件来表示的
...
不能关闭已经关闭了的文件。因此应如上例一样检查`close()`函数的返回值
...
返回值是读取的字节数量，是一个 `ssize\_t` 类型（其实就是一个有符号整型），如果 `nbytes < 0` 那么表示出错。`nbytes < sizeof(buf)` 这种情况(short counts) 是可能发生的，而且并不是错误。
...
RIO包通过不断调用`read`和`write`函数提供高效且健壮的接口，能用于自动处理short count的情况，尤其常用于网络编程。
...
![file-20251124193111207](assets/file-20251124193111207-20260104152052-g3p76ay.png)

```
void rio_readinitb(rio_t *rp, int fd){ /* 初始化缓冲区 */
	rp->rio_fd = fd;
	rp->rio_cnt = 0;
...
static ssize_t rio_read(rio_t *rp, char *usrbuf, size_t n){
	while(rp->rio_cnt <= 0){ // nothing to read, need to calll syscall
...
![file-20251124202122608](assets/file-20251124202122608-20260104152052-mwnvxni.png)
了解了具体的结构之后，我们来看看内核是如何表示已打开的文件的。其实过程很简单，每个进程都有自己的描述符表(Descriptor table)，然后 Descriptor 1 指向终端，Descriptor 4 指向磁盘文件，如下图所示：

![file-20251124202250229](assets/file-20251124202250229-20260104152052-2cjwn9h.png)
这里有一个需要说明的情况，就是使用 `fork`。子进程实际上是会继承父进程打开的文件的。在 fork 之后，子进程实际上和父进程的指向是一样的，这里需要注意的是会把引用计数加 1，如下图所示

![file-20251124202336892](assets/file-20251124202336892-20260104152052-a7akz29.png)
