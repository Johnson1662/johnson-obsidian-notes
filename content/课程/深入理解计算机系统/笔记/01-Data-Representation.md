...
**字长（word size）**：指通用寄存器的长度，也就是一般情况下一次计算所能处理的最多的数据位数

- 对于一个字长为 $w$ 位的机器而言，通用寄存器可以存储 $w$ 位数据，虚拟地址空间的理论最大大小为 $2^w$ 个字节
- 需要注意的是，实际的虚拟地址和物理地址范围并不总是等于 $2^w$，具体实现会因架构、模式等因素而有所不同
...
![mem](assets/01-Data-Representation/mem.svg)
...
（-1 在计算机中表示为全 1，即 $\text{UMax}_w$，而 0u 表示为全 0 的无符号数）
...
![T2U&U2T](assets/01-Data-Representation/T2U&U2T.png)
...
4. 从短、有符号的 `short` 型变量 `s1` 到长、无符号的 `unsigned` 型变量 `u1`，先符号扩展，再看作无符号数，得到 `0xffff8313`
...
![overflow](assets/01-Data-Representation/overflow.png)
...
![signed_overflow](assets/01-Data-Representation/signed_overflow.png)
...
![IEEE 754](assets/01-Data-Representation/IEEE.png)
...
1. 显然阶码位要尽可能的大，才会让误差变大，由于不考虑 `Inf`，所以最大时，阶码位为 `11111110`，即 $2^{8} - 2$，于是
 $E = [e] - Bias = (2^8 - 2) - (2^7 - 1) = 2^7 - 1 = 127$
