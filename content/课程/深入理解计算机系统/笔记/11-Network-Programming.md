...
![client-server](/11-Network-Programming/client_server.png)

客户端和服务端是 **进程**，而不是 **主机**，它们可以在同一台主机上，也可以在不同的主机上。
...
网络是一种 **I/O** 设备，通常使用 DMA 传输。

网络是按照地理远近组成的层次系统。


![network_io](/11-Network-Programming/network_io.png)
...
![lan](/11-Network-Programming/lan.png)


桥接 LAN


![wan](/11-Network-Programming/wan.png)
...
互联网（internet）：路由器连接若干 LAN 和 WAN

全球 IP 因特网（Internet）：是互联网（internet）的具体实现
...
- 命名机制
- 传送机制：运行在主机和路由器上，控制主机和路由器协同工作。
一个包是由 **包头** 和 **有效载荷** 组成的，包头包括了传输所需信息，如源主机地址、目的主机地址
...
<img src="/11-Network-Programming/internet_transfer.png"/>
...
![internet](/11-Network-Programming/internet.png)
...
![tcp_vs_udp](/11-Network-Programming/tcp_vs_udp.png)
...
![domain](/11-Network-Programming/domain.png)
...
套接字是连接的一个 **端点**，一对套接字组成一个连接。

套接字地址形如 `(IP 地址:端口号)`

套接字对形如 `(客户端 IP:客户端端口, 服务端 IP:服务端端口)`，其唯一确定了一个连接。

![socket_pair](/11-Network-Programming/socket_pair.png)
...
![socket_interface](/11-Network-Programming/socket_interface.png)
...
2. 然后我们需要用 `socket` 创建一个套接字，但是这只是填入了对方的基础信息，我们不确定对方的状态，所以此时套接字是 **不可用** 的。
3. 我们需要调用 `connect` 来尝试建立连接，若能正常建立，则得知此时套接字是 **可用** 的。
...
2. 然后我们需要用 `socket` 创建一个套接字，用于监听客户端的连接请求，此时，这个套接字依旧是 **不可用** 的。
...
![getaddrinfo_res](/11-Network-Programming/getaddrinfo_res.png)
...
![listen_fd_vs_connected_fd](/11-Network-Programming/listen_fd_vs_connected_fd.png)
