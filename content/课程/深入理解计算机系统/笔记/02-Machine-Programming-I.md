...
- Architecture（架构）：也称为 ISA（指令集架构，Instruction Set Architecture）。
...
![image](assets/02-Machine-Programming-I/registers.png)
backgroundSize: 80%
...
操作数主要各式如下图。注意：
- 基址和变址寄存器必须是 64 位寄存器 （记忆：64 位系统） → `(%eax)` 不合法
- 比例因子必须是 1、2、4、8 （记忆：二进制） → `(,%rax,3)` 不合法

![operand](assets/02-Machine-Programming-I/operand.png)
...
- `mov` 指令的两个操作不能都是内存类型，D 表示 destination / 目的地，S 表示 source / 源。
- 寄存器大小必须和指令后缀匹配。
- \* `movl` 指令以寄存器为目的时，会将高位 4 字节清零。
...
![image](assets/02-Machine-Programming-I/runtime-memory.png)
backgroundSize: 80%
...
![push-pop](assets/02-Machine-Programming-I/push-pop.png)
...
`leaq` 指令：load effective address，意为“加载有效地址”。**但实际上主要用于计算**
...
注意：只有这个寄存器可以存移位量！（记忆：`cl` → count lower）
...
- CF（Carry Flag）：当两个 unsigned 类型的数作运算因 **进位/借位** 而发生溢出时置为 1，否则置为 0
...
计算 `s2 - s1`，但不同于 `subq` 会修改 `s2` 的值，`cmpq` 只会影响条件码的值。
...
Q：如果想要设置全部 8 Byte，该使用什么指令？ A：`movzbl` / `movzbq`
...
注意，其中一个加数的是 **下一条指令** 的地址！

![jmp-offset](assets/02-Machine-Programming-I/jmp-offset.png)
...
4. * 为什么 1 中编译的可执行文件无法在同一处理器上的 Windows 系统上执行？分歧主要发生在哪一步骤中？
