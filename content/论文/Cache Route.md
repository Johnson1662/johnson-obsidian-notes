...
* 同一 business 固定服务器 ：缓存命中高，但流量倾斜导致热点服务器，恶化 p99（第 99 百分位延迟）。
...
两个 32B workload（工作负载）中，缓存收益不足时，CacheRoute 分别只有 Flat-LB 的 **0.50–0.67×** 容量和 **1.0×** 容量。
...
因此作者建议上线前用 **Shadow Replay（影子流量回放）**真实测量缓存命中、负载和 p99，确认性能改善后再启用。
