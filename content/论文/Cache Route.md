...
作者模拟的是多租户聊天服务，比如很多公司的客服机器人。每个 business（业务）都有一段固定上下文，因此同一业务的不同聊天会重复使用这部分 Prefix。
...
平均 Prompt 大约 1200 token，但真正和路由有关、能够稳定复用的是每个业务自己的约 180 token 上下文。全局模板去哪台机器都可能缓存；RAG 获取的 few-shot 内容又经常变化，因此并不是主要路由优化对象。 
