...
Prefix caching is now standard in LLM serving engines \[[Kwon et al.(2023)](CacheRoute%20Planned%20Prefix-Affinity%20Routingfor%20Large-Scale%20LLM%20Serving.md#bib.bibx1), [Zheng et al.(2024)](CacheRoute%20Planned%20Prefix-Affinity%20Routingfor%20Large-Scale%20LLM%20Serving.md#bib.bibx2)\], but the cache cannot control where a request lands. A cache-blind balancer spreads a recurring prefix across many destinations and lengthens the return time to each cache. Pinning a key to one destination restores locality, but maps key skew directly onto server queues. Cache-aware routers instead react to current cache and load state \[[Srivatsa et al.(2024)](CacheRoute%20Planned%20Prefix-Affinity%20Routingfor%20Large-Scale%20LLM%20Serving.md#bib.bibx3), [Yuan et al.(2026)](CacheRoute%20Planned%20Prefix-Affinity%20Routingfor%20Large-Scale%20LLM%20Serving.md#bib.bibx4)\]; once a destination fills, however, spilling the next request also forfeits the warm prefix.
...
Each admitted key contributes $k_{b}$ jobs of size $\lambda_{b}/k_{b}$. After initializing destinations with their expected cold-tail share, the planner visits admitted keys in decreasing-rate order and assigns each job to the least-loaded eligible destination. The procedure follows classic LPT scheduling \[[Graham(1969)](CacheRoute%20Planned%20Prefix-Affinity%20Routingfor%20Large-Scale%20LLM%20Serving.md#bib.bibx5)\], but the standard approximation bound does not apply to our split jobs and distinct-destination constraint. During the control interval, admitted traffic chooses the least-loaded member of its fixed $T(b)$; other traffic uses power-of-two choices across the fleet \[[Mitzenmacher et al.(2001)](CacheRoute%20Planned%20Prefix-Affinity%20Routingfor%20Large-Scale%20LLM%20Serving.md#bib.bibx6)\].
...
We compare against Flat-LB (power-of-two-choices), sticky consistent hashing \[[Karger et al.(1997)](CacheRoute%20Planned%20Prefix-Affinity%20Routingfor%20Large-Scale%20LLM%20Serving.md#bib.bibx7)\], consistent hashing with bounded loads (CHWBL) \[[Mirrokni et al.(2018)](CacheRoute%20Planned%20Prefix-Affinity%20Routingfor%20Large-Scale%20LLM%20Serving.md#bib.bibx8)\], a DualMap-style two-candidate cache/load policy \[[Yuan et al.(2026)](CacheRoute%20Planned%20Prefix-Affinity%20Routingfor%20Large-Scale%20LLM%20Serving.md#bib.bibx4)\], and a Preble-style prefix-history and live-load policy \[[Srivatsa et al.(2024)](CacheRoute%20Planned%20Prefix-Affinity%20Routingfor%20Large-Scale%20LLM%20Serving.md#bib.bibx3)\]. The latter three are reimplemented in our harness under a common interface; our absolute results should not be read as reproductions of their original systems. CHWBL uses $\epsilon=0.25$, selected by an offline sweep, and the Preble-style policy uses a 1.5 load-cap factor.
